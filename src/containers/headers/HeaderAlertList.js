...
import {selectHeaderAlertNotificationItem} from '../../actions/selectHeaderAlertNotificationItem'

const grayTextStyle = {color:"#888"};
const listItemStyle = {border:"0px solid white",paddingLeft:"0em",paddingRight:"0em",cursor:"pointer"};
class HeaderAlertList extends Component{
    createListItem(){
        return this.props.alertNotifications.map((item) => {
            return (
                <ListGroupItem
                  style={listItemStyle}
                  key={item.messageID}
...
{
    return bindActionCreators({clickItem : selectHeaderAlertNotificationItem },dispatch);
}

export default connect(mapStateToProps,matchDispatchToProps)(HeaderAlertList);
